...
window.cancelarDesenhoBarra = function() {
    if (pontoInicialBarra) {
        ...
        console.log("🔄 Ponto azul cancelado por mudança de ferramenta.");
    }
}
...
for (let x = 0; x <= width; x += tamanhoGrelha) {
    for (let y = 0; y <= height; y += tamanhoGrelha) {
        ...
        const ponto = new Konva.Circle({
            ...
            strokeWidth: 10, // tamanho da borda
        ...
        ponto.on('click tap', function () {
            ...
            if (window.ferramentaAtual === 'Inserir Barra') {
                ...
                if (pontoInicialBarra === null) {
                    ...
                    // --- NOVA LÓGICA: INSPEÇÃO E SELEÇÃO DA BARRA ---
...
document.addEventListener('keydown', function(event) {
    if ((event.key === 'Delete' || event.key === 'Backspace') && window.ferramentaAtual === 'Selecionar') {
        if (window.barraSelecionada) {
            ...
            layerEstrutura.draw();
            console.log("🗑️ Barra excluída com sucesso!");
        }
    }
});
...
const eixoX = new Konva.Arrow({
    points: [origemX, origemY, width - 20, origemY], 
    pointerLength: 10,
    pointerWidth: 10,
    fill: '#ff3333',     
    stroke: '#ff3333',   
...
const eixoY = new Konva.Arrow({
    points: [origemX, origemY, origemX, height - 20], 
    pointerLength: 10,
    pointerWidth: 10,
    fill: '#3333ff',     
    stroke: '#3333ff',   
